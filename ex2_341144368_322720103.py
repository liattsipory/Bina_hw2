...
class InfinitePirateAgent:
    ...
    def __init__(self, initial, gamma):
        ...
        self.value_iteration_table, self.policy_table, self.t_star = self.value_iteration(state, self.map)
        #print(self.t_star)
...
each combination should combine all the treasures and all the marine ships
                ...
                for capacity in range(3):
                    for m_combination in combinations_of_paths:
                        for t_combination in combinations_of_tlocs:
                            ...
                            possible_states.append(current_state)
                            hash_state = self.hashable_vi(current_state)
                            V_row[hash_state] = self.r(current_state)
        V.append(copy.deepcopy(V_row))
        # print('V0')
        #print(V[0])
        t = 0
        pi_row = {}
            ...
            for s in possible_states:
                ...
                for action in self.possible_actions(s):
                    ...
                    for s_prime in prob_dict.keys():
                        value += prob_dict[s_prime] * V[t - 1][s_prime]
                    value *= self.gamma
                    if value > V_row[hash_state]:
                        pi_row[hash_state] = action
                        V_row[hash_state] = value
                V_row[hash_state] += self.r(s)
            # creating V[t] and pi[t]
            V.append(copy.deepcopy(V_row))
            pi.append(copy.deepcopy(pi_row))
            differences = []
            for key in V[t]:
                differences.append(V[t][key] - V[t-1][key])
    ...
    def act(self, s):
        ...
        #current_turn = s['turns to go']
        hash_state = self.hashable_vi(state)
        best_action = self.policy_table[self.t_star-1][hash_state]
        action_name = best_action[0]
        best_action_extended = []
        for p in self.pirate_names:
            if action_name == 'sail':
                ...
                best_action_extended.append(best_action)
        #print('turns to go - infinite, best action - ', best_action_extended)
        return tuple(best_action_extended)

    def value(self, state):
        vi_state = self.create_state_from_input(state)
        hash_state = self.hashable_vi(vi_state)
        return self.value_iteration_table[self.t_star-1][hash_state]
